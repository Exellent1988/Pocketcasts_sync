...
class Episode:
    def __init__(self, data: dict, podcast, api):
        ...
        self.playing_status = data.get('playingStatus')
    ...
    def details(self):
        endpoint = _get_endpoint("single_episode")
        url = _make_url(base=self._api._api_base,
                        endpoint=endpoint)
        json_data = self._api._post(url=url,
                                    data={'uuid': f"{self.uuid}" })
        # print(f"{data}")
        
        if json_data.ok and json_data.json():
            json_data = json_data.json()
            self.uuid = json_data["uuid"]
            self.url = json_data["url"]
            self.published_date = json_data["url"]
            self.duration = json_data["duration"]
            self.file_type = json_data["fileType"]
            self.title = json_data["title"]
            self.size = json_data["size"]
            self.playing_status = json_data["playingStatus"]
            self.current_position = json_data["playedUpTo"]
            self.starred = json_data["starred"]
            self.podcast_uuid = json_data["podcastUuid"]
            self.podcast_title = json_data["podcastTitle"]
            self.episode_type = json_data["episodeType"]
            self.season = json_data["episodeSeason"]
            self.number = json_data["episodeNumber"]
            self.deleted = json_data["isDeleted"]
        else:
            raise Exception
        return self
